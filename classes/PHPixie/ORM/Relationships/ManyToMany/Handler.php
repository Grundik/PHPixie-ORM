...
class Handler{
	...
	public function relationship_side_query($side, $condition, $property) {
		...
		return $config["{$side}_repo"]->query()
									->related($config[$side.'_property'], $condition);
	}
	
	protected get_pivot_side_subquery($side, $opposing_side, $config, $opposing_query, $alias) {
	
		$query = $this->db->query('select', $config['pivot_connection'])
						->fields(array($config["{$side}_pivot_key"]))
						->target($config['pivot']);
	...
	protected function get_side_ids_subquery($side, $config, $conditions, $alias) {
		$repo = $config["{$side}_repo"];
		
		$query = $this->db->query('select', $repo->connection_name())
	...
	protected function process_side_relationship($side, $opposing_side, $config, $query, $group, $relationship, $plan) {
		...
		$this->id_strategy->add_condition($query, 'and', false, $config["{$opposing_side}_pivot_key"], $opposing_subquery);
		$this->id_strategy->add_condition($query, $group->logic, $group->negated(), $config["{$side}_repo"]->id_field(), $pivot_subquery);
	}
	...
	protected function process_remove($config, $side, $opposing_side, $model, $collection) {
		...
		$side_id = $model->id();
		$ids = $collection->field($config["{$side}_repo"]->model_name());
